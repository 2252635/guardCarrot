			...
			label = static_cast<BlockType>(currentBlock->get_label());
				...
				case ROAD:
				{
					/*for (auto currentMonster : monsterManager->getVec())
						if (pos.x >= (currentMonster->getMonster()->getPosition().x - BLOCK_SIZE / 2) && pos.x <= (currentMonster->getMonster()->getPosition().x + BLOCK_SIZE / 2)
							&& pos.y >= (currentMonster->getMonster()->getPosition().y - BLOCK_SIZE / 2) && pos.y >= (currentMonster->getMonster()->getPosition().y + BLOCK_SIZE / 2) && !checkTarget()) {
							if (checkTarget()) {
