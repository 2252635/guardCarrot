...
bool MapPurple::init()
{
        ...
        if (e) {
            ...
            if (removingTopMostNode) {
                ...
                if (label == NO_TOWER) {
                    ...
                    auto towerOption1 = MenuItemImage::create(bottleImage+ PHOTOPOSTFIX, bottleImage + PHOTOPOSTFIX, CC_CALLBACK_1(MapPurple::onTowerSelected, this, TowerType::BOTTLE, targetPos,bottleFlag));
                    auto towerOption2 = MenuItemImage::create(starImage + PHOTOPOSTFIX, starImage + PHOTOPOSTFIX, CC_CALLBACK_1(MapPurple::onTowerSelected, this, TowerType::STAR, targetPos, starFlag));
                    auto towerOption3 = MenuItemImage::create(shitImage + PHOTOPOSTFIX, shitImage + PHOTOPOSTFIX, CC_CALLBACK_1(MapPurple::onTowerSelected, this, TowerType::SHIT, targetPos, shitFlag));
                    ...
                    auto towerOption1 = MenuItemImage::create(upGradeImage, upGradeImage, CC_CALLBACK_1(MapPurple::onTowerDestroySelected, this, targetTower, 1,targetPos));
                    auto towerOption2 = MenuItemImage::create(TOWERDEADBUTTON, TOWERDEADBUTTON, CC_CALLBACK_1(MapPurple::onTowerDestroySelected, this, targetTower, 0,targetPos));
                    ...
                    auto carrotOption = MenuItemImage::create(upImage, upImage, CC_CALLBACK_1(MapPurple::onCarrotSelected, this,targetPos));
    ...
    schedule(CC_SCHEDULE_SELECTOR(MapPurple::checkVariableChange, "update", 0.1f, -1, 0));
    this->removeChild(gameManager->getProperty().getMoneyLabel());
    return true;
}

void MapPurple::checkVariableChange(float dt)
{
    game();
    if (isEnd == 1) {
...
void MapPurple::onTowerDestroySelected(Ref* sender, TowerBase* targetTower, bool option,Vec2 pos)
{
...
void MapPurple::onTowerSelected(Ref* sender, TowerType type, Vec2 pos,bool flag) {
    // 从 sender 获取菜单项和菜单
    // 移除菜单
    Node* topMostNode = this->getChildByTag(kTowerSelectMenuTag);
...
void MapPurple::onCarrotSelected(Ref* sender, Vec2 pos) {
    // 从 sender 获取菜单项和菜单
    // 移除菜单
    Node* topMostNode = this->getChildByTag(kTowerSelectMenuTag);
...
void MapPurple::game()
{
    ...
    Vec2 targetPosition(origin.x + 16 * BLOCK_SIZE + BLOCK_SIZE / 2, origin.y + 6 * BLOCK_SIZE + BLOCK_SIZE / 2);
